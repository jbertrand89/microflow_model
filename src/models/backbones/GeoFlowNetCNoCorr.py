...
def get_GeoFlowNetCNoCorr_model(batchNorm=False): 
    model = GeoFlowNetCNoCorr(batchNorm=batchNorm)
    return model


class GeoFlowNetCNoCorr(nn.Module):
    """
    GeoFlowNetC_NoCorr is a modified version of GeoFlowNetC that does not use correlation.
    """
    expansion = 1

    def __init__(self, batchNorm=True):
        super(GeoFlowNetCNoCorr, self).__init__()
        ...
        self.conv4_1 = conv(self.batchNorm, 1024,  512)
    ...
    def forward(self, x):
        ...
        in_conv4_1 = torch.cat([out_conv4a, out_conv4b], dim=1)  # b x 473 x 64 x 64
