...
def get_iterative_separated_weights_model(
        device, model_name, batch_norm, max_iterations, filenames, args, trained_end2end=False):
    model = IterativeSeparatedWeights(
        device, model_name, batch_norm, max_iterations, filenames, trained_end2end,
...
class IterativeSeparatedWeights(nn.Module):
    expansion = 1

    def __init__(
            self, device, model_name, batch_norm, max_iterations, filenames, trained_end2end, repeat_model=False, repeat_first_iteration=False):
        super(IterativeSeparatedWeights, self).__init__()
        self.max_iterations = max_iterations
        self.repeat_model = repeat_model
        self.repeat_first_iteration = repeat_first_iteration
        ...
        if not trained_end2end:
            self.load_data_separately(filenames)

    def load_data_separately(self, filenames):
        """ For loading each iteration model trained separately (or stacked)
        """
    ...
    def forward(self, x, ptv=None, args=None, save=False, frame_labels=False, crs_meta_data=None, transform_meta_data=None):
        ...
        for iteration in range(self.max_iterations):
            ...
            if self.repeat_first_iteration:
                iteration_optical_flows.append(self.iteration_models[0](iteration_inputs[iteration])[0])
            else:
            ...
            if save:  
                ...
                # For tiff visualization, save the denoised version at each iteration, though only the last iteration is denoised in the model
